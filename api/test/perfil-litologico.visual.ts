...
function svg(perfil: PerfilLitologico, rango: PerfilLitologico["rangos"][number]) {
  ...
  const aportes = perfil.aportes.filter((a) => a.profundidad_m >= rango.desde_m && a.profundidad_m <= rango.hasta_m).map((a) => `<circle cx="282" cy="${y(a.profundidad_m)}" r="6" class="aporte"/><text x="294" y="${y(a.profundidad_m) - 6}" class="azul">Aporte ${a.profundidad_m} m</text>`).join("");
  return `<h2>${rango.desde_m}-${rango.hasta_m} m</h2><svg viewBox="0 0 760 820"><defs><pattern id="diagonal" width="8" height="8" patternUnits="userSpaceOnUse"><path d="M0 8L8 0"/></pattern><pattern id="diagonal-inversa" width="8" height="8" patternUnits="userSpaceOnUse"><path d="M0 0L8 8"/></pattern><pattern id="cruz" width="8" height="8" patternUnits="userSpaceOnUse"><path d="M0 4H8M4 0V8"/></pattern><pattern id="puntos" width="8" height="8" patternUnits="userSpaceOnUse"><circle class="punto" cx="4" cy="4" r="1.2"/></pattern><pattern id="horizontal" width="8" height="8" patternUnits="userSpaceOnUse"><path d="M0 4H8"/></pattern><pattern id="vertical" width="8" height="8" patternUnits="userSpaceOnUse"><path d="M4 0V8"/></pattern></defs>${guias}<rect x="90" y="70" width="180" height="700" fill="#fafafa" stroke="#111"/>${capas}${aportes}</svg>`;
}
